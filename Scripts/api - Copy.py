...
def sf_schema():    
       ...
       with open(r"Connections\\Outbound\\{}.json".format(outbound), 'w') as outbound:
        ...
        sf_connect(json1["account"],json1["user"],json1["password"],json1["warehouse"],json1["database"],json1["role"])
...
def tbl_names():    
    ...
    if r[0] in In_bnd:
        ...
        sql_cntrl = "SELECT TABNAME FROM SAPABAP1.DD02L WHERE TABCLASS = 'TRANSP' AND CONTFLAG = 'A' AND TABNAME IN (SELECT TABLE_NAME FROM SYS.TABLES WHERE SCHEMA_NAME = "+ "'"+schema_name+"'" +") ORDER by TABNAME ASC LIMIT 10000"
#        hana_conn = hana_connect(j["address"],j["port"],j["user"],j["password"])
...
def tbl_names_Outbound():    
    ...
    if r[0] in Out_bnd :
        ...
        data = {"file_name":file_name}
        with open(r"Connections\Outbound\data.json","w") as fn:
        	json.dump(data,fn)
        with open(file_name) as f:
            j1 = json.load(f) 
            print(j1)
        ...
        sql_cntrl = "select TABLE_NAME from INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA="+ "'"+schema_name+"'" +" "     
        sf_cursor.execute(sql_cntrl)
        df = sf_cursor.fetch_pandas_all()
        tab_names = df["TABLE_NAME"].to_list()
    return jsonify({"table_names":tab_names})
...
#         print("$$$$$$$$$$$$",result)  
# #    table_result = table_schema_migrate(user_table_names)
#     table_result = table_schema_migrate(result)
# #        print(table_result)
    
    
# #    table_result = "Table already exists in Snowflake!"
#     return  table_result
